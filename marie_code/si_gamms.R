...
df_summary <- df %>%
  ...
  summarise_at(vars(LC_pred, TP_pred, Area_ha, summer_air_temp, LaTro_CUE, total_pred_CUE, sampling_year, secchi, pLittoral, total_p, SDI), list(mean = mean, sd = sd)) %>%
...
lc_pred_3 <- ggplot() +
  ...
  theme_classic() +
  theme(axis.text = element_text(size = 14), axis.title = element_text(size = 16)) +
  labs(x = "Other Predator CPUE (ln)", y = "Mean PNC (logit)")
lc_pred_3


##Linear Models w/ other lake attributes
meanPNC_lm_SDI <- lm(logit_lc_pred ~ log(SDI_mean), data = df_summary)
summary(meanPNC_lm_SDI)

meanPNC_lm_secchi <- lm(logit_lc_pred ~ log(secchi_mean), data = df_summary)
summary(meanPNC_lm_secchi)

meanPNC_lm_plit <- lm(logit_lc_pred ~ pLittoral_mean, data = df_summary)
summary(meanPNC_lm_plit)

meanPNC_lm_tp <- lm(logit_lc_pred ~ log(total_p_mean), data = df_summary)
summary(meanPNC_lm_tp)
...
tp_pred_3 <- ggplot() +
  ...
  theme_classic() +
  theme(axis.text = element_text(size = 14), axis.title = element_text(size = 16)) +
  labs(x = "Other Predator CPUE (ln)", y = "Mean TP")
tp_pred_3

##Linear Models w/ other lake attributes
meanTP_lm_SDI <- lm(TP_pred_mean ~ log(SDI_mean), data = df_summary)
summary(meanTP_lm_SDI)

meanTP_lm_secchi <- lm(TP_pred_mean~ log(secchi_mean), data = df_summary)
summary(meanTP_lm_secchi)

meanTP_lm_plit <- lm(TP_pred_mean ~ pLittoral_mean, data = df_summary)
summary(meanTP_lm_plit)

meanTP_lm_tp <- lm(TP_pred_mean ~ log(total_p_mean), data = df_summary)
summary(meanTP_lm_tp)
...
##mean -- 52 lakes 
mean_52 <- ggarrange(tp_area_1, tp_air_1, lc_area_1, lc_air_2, nrow = 2, ncol = 2, labels = c("a)", "b)", "c)", "d)"))
mean_52
...
lc_range_lt_1 <- visreg(dPNC_gamm1.2, "log_pred_cue", partial = TRUE, gg = TRUE)
...
##Linear Models w/ other lake attributes
dPNC_lm_SDI <- lm(log_dX_range ~ log(SDI), data = siber_df_final)
summary(dPNC_lm_SDI)

dPNC_lm_secchi <- lm(log_dX_range ~ log(secchi), data = siber_df_final)
summary(dPNC_lm_secchi)

dPNC_lm_plit <- lm(log_dX_range ~ pLittoral, data = siber_df_final)
summary(dPNC_lm_plit)

dPNC_lm_tp <- lm(log_dX_range ~ log(total_p), data = siber_df_final)
summary(dPNC_lm_tp)
...
##Linear Models w/ other lake attributes
dTP_lm_SDI <- lm(log_TP_range ~ log(SDI), data = siber_df_final)
summary(dTP_lm_SDI)

dTP_lm_secchi <- lm(log_TP_range ~ log(secchi), data = siber_df_final)
summary(dTP_lm_secchi)

dTP_lm_plit <- lm(log_TP_range ~ pLittoral, data = siber_df_final)
summary(dTP_lm_plit)

dTP_lm_tp <- lm(log_TP_range ~ log(total_p), data = siber_df_final)
summary(dTP_lm_tp)
...
##Linear Models w/ other lake attributes
SEA_lm_SDI <- lm(log_SEAc ~ log(SDI), data = siber_df_final)
summary(SEA_lm_SDI)

SEA_lm_secchi <- lm(log_SEAc ~ log(secchi), data = siber_df_final)
summary(SEA_lm_secchi)

SEA_lm_plit <- lm(log_SEAc ~ pLittoral, data = siber_df_final)
summary(SEA_lm_plit)

SEA_lm_tp <- lm(log_SEAc ~ log(total_p), data = siber_df_final)
summary(SEA_lm_tp)
...
SDNND_lm_SDI <- lm(log_SDNND ~ log(SDI), data = siber_df_final)
summary(SDNND_lm_SDI)

ggplot(siber_df_final, aes(x = log(SDI), y = log_SDNND)) +
  geom_point() +
  geom_smooth(method = "lm")

SDNND_lm_secchi <- lm(log_SDNND ~ log(secchi), data = siber_df_final)
summary(SDNND_lm_secchi)

SDNND_lm_plit <- lm(log_SDNND ~ pLittoral, data = siber_df_final)
summary(SDNND_lm_plit)

SDNND_lm_tp <- lm(log_SDNND ~ log(total_p), data = siber_df_final)
summary(SDNND_lm_tp)
...
extract_gam_summary_2 <- function(model, response_name) {
  ...
  if (!is.null(model_summary$p.table)) {
    ...
    fixed <- fixed %>%
      mutate(across(c(Estimate, Std_Error, Stat_Value, P_Value, Adj_R2, Dev_Expl, AIC, DF), as.character))
  } else {
    fixed <- tibble(
      Response = response_name,
  ...
  smooth_terms <- smooth %>%
    ...
    select(Response, Term, Estimate, Std_Error, Stat_Value, P_Value, Adj_R2, Dev_Expl, AIC, DF) %>%
    mutate(across(c(Estimate, Std_Error, Stat_Value, P_Value, Adj_R2, Dev_Expl, AIC, DF), as.character))
...
###Tables for linear model results
extract_lm_summary <- function(model, response_name) {
  model_summary <- summary(model)
  ...
  # Fill fit info in first row only
  output$Adj_R2[1] <- adj_r2
#  output$AIC[1] <- aic_val
  output$df[1] <- df_resid
  
  # Reorder and convert to character
  output <- output %>%
    select(Response, Term, df, Estimate, Std_Error, `F`,  Adj_R2, p) %>%
    mutate(across(c(Estimate, df, Std_Error, `F`, Adj_R2, p), as.character))
  
  return(output)
}


##create nested lists for each of the linear model outputs for the 4 other lake attributes
models_SDI = list(`mean TP` = meanTP_lm_SDI,
                 `mean PNC (logit)` = meanPNC_lm_SDI,
                  `dTP (log)` = dTP_lm_SDI,
                  `dPNC (log)` = dPNC_lm_SDI,
                  `SEAc (log)`  = SEA_lm_SDI,
                  `SDNND (log)` = SDNND_lm_SDI)

# models_nested is your named list, e.g., list(SDNND = gam1, LC = gam2, ...)
lm_SDI <- map_dfr(names(models_SDI), function(name) {
  extract_lm_summary(models_SDI[[name]], response_name = name)
})
...
write.csv(lm_SDI, "lm_tables/lm_output_SDI.csv")


##secchi depth
models_secchi = list(`mean TP` = meanTP_lm_secchi,
                  `mean PNC (logit)` = meanPNC_lm_secchi,
                  `dTP (log)` = dTP_lm_secchi,
                  `dPNC (log)` = dPNC_lm_secchi,
                  `SEAc (log)`  = SEA_lm_secchi,
                  `SDNND (log)` = SDNND_lm_secchi)

# models_nested is your named list, e.g., list(SDNND = gam1, LC = gam2, ...)
lm_secchi <- map_dfr(names(models_secchi), function(name) {
  extract_lm_summary(models_secchi[[name]], response_name = name)
})
...
write.csv(lm_secchi, "lm_tables/lm_output_secchi.csv")

##pLittoral
models_plit = list(`mean TP` = meanTP_lm_plit,
                  `mean PNC (logit)` = meanPNC_lm_plit,
                  `dTP (log)` = dTP_lm_plit,
                  `dPNC (log)` = dPNC_lm_plit,
                  `SEAc (log)`  = SEA_lm_plit,
                  `SDNND (log)` = SDNND_lm_plit)

# models_nested is your named list, e.g., list(SDNND = gam1, LC = gam2, ...)
lm_plit<- map_dfr(names(models_plit), function(name) {
  extract_lm_summary(models_plit[[name]], response_name = name)
})
...
write.csv(lm_plit, "lm_tables/lm_output_pLit.csv")

##total phosphorous
models_tp = list(`mean TP` = meanTP_lm_tp,
                   `mean PNC (logit)` = meanPNC_lm_tp,
                   `dTP (log)` = dTP_lm_tp,
                   `dPNC (log)` = dPNC_lm_tp,
                   `SEAc (log)`  = SEA_lm_tp,
                   `SDNND (log)` = SDNND_lm_tp)

# models_nested is your named list, e.g., list(SDNND = gam1, LC = gam2, ...)
lm_tp<- map_dfr(names(models_tp), function(name) {
  extract_lm_summary(models_tp[[name]], response_name = name)
})
...
write.csv(lm_tp, "lm_tables/lm_output_tp.csv")
