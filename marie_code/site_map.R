...
mapview(sites_coord, zcol = "Main_Analysis", col.regions = c("purple", "red"), legend = TRUE, map.types = "Esri.NatGeoWorldMap")


52-28
##measure distance between furthest apart lakes 
coords <- lake_df %>%
  select(Longitude, Latitude)

dist_matrix <- distm(coords, fun = distHaversine)
max_dist_meters <- max(dist_matrix)
max_dist_km <- max_dist_meters/1000
